...
$(function () {
    ...
    windowNW < 500 ? $("[name=rencaiPhoto]").css({width: '100%'}) : $("[name=rencaiPhoto]").css({width: '300px'});
    //设置人才开户行的宽度
    setKuan('rckhh');
    setKuan('zszhuanye');
    setKuan('zjkhh');
    ...
    //默认为建造师，的表单
    hideXxDiv('zsbgDiv-yjzmDiv-zyddDiv-jpzmDiv-hongtouDiv-psbiaoDiv');
    ...
    $("#addDanwe").click(function () {
        var danweiGroup = $("  <div  >\n" +
            ...
            "                                        <span class='input-group-addon'>\n" +
            "                                  <span class='glyphicon glyphicon-calendar'></span>\n" +
            "                                 </span></div></div><span id='delete-' style='height:30px;' class='btn btn-danger '>删除</span>");
        ...
        var optins = $("#sydanwei").children().clone();
        var last = $("[name=sydanwei]").length - 1;
        $("[name=sydanwei]:eq(" + last + ")").append(optins);
